...
def load_label_training_dataset(dataset_type):
    filenames = [str(trainint_set_path / x) for x in os.listdir(trainint_set_path) if "not_relevant" in x and dataset_type in x]
    dataset = tf.data.TFRecordDataset(filenames)
...
def load_label_validation_dataset(dataset_type):
    filenames = [str(validation_set_path / x) for x in os.listdir(validation_set_path) if "not_relevant" in x and dataset_type in x]
    dataset = tf.data.TFRecordDataset(filenames)
