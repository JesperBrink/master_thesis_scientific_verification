...
def load_relevance_training_dataset():
...
def load_relevance_validation_dataset():
    filenames = [str(validation_set_path / x) for x in os.listdir(validation_set_path)]
    dataset = tf.data.TFRecordDataset(filenames)
...
def load_label_training_dataset():
    filenames = [str(trainint_set_path / x) for x in os.listdir(trainint_set_path)]
    dataset = tf.data.TFRecordDataset(filenames)
...
def load_label_validation_dataset():
    filenames = [str(validation_set_path / x) for x in os.listdir(validation_set_path)]
    dataset = tf.data.TFRecordDataset(filenames)
