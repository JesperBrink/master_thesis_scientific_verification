...
train_batch_size = 2
num_epochs = 4
model_save_path = 'output/cross-encoder-stsb-nli-allenai-scibert-scivocab-uncased-'+datetime.now().strftime("%Y-%m-%d_%H-%M-%S")

#############################################################################################################
# Needs to replace the classification layer, to replace it with a layer with a single output instead of three
model = CrossEncoder('output/cross-encoder-nli-allenai-scibert-scivocab-uncased', num_labels=3)
