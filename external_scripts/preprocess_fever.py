"""
SCRIPT TAKEN FROM: https://github.com/allenai/scifact/blob/master/verisci/training/preprocess_fever.py
...
parser.add_argument(
    "--wiki-folder", type=str, required=True, help="Path to wiki-pages folder"
)
parser.add_argument(
    "--input",
    type=str,
    required=True,
    help="Path to train.jsonl, paper_dev.jsonl or paper_test.jsonl",
)
parser.add_argument(
    "--output",
    type=str,
    required=True,
    help="Path to output jsonl, i.e fever_train.jsonl",
)
...
for filename in tqdm(sorted(os.listdir(args.wiki_folder))):
    for wiki_doc in jsonlines.open(os.path.join(args.wiki_folder, filename)):
        ...
        wiki_sentences = [
            s[s.find("\t") + 1 :].replace("\t", " ") for s in wiki_sentences
        ]
...
for claim_doc in tqdm(claims):
    ...
    for es in claim_doc["evidence"]:
        evidence_sentences = [
            sentence_mapping[(e[2], e[3])] for e in es if e[2] in wiki_dict.keys()
        ]
